...
It currently contains fourteen independently packable packages:
...
Angular bindings will own those types and renderer integration. AJV is build-time-only for runtime
...
therefore quarantined to the two JSON Forms React adapter projects only. The workspace default and
all framework-neutral packages retain full declaration checking, and the validator rejects any
additional quarantine.

The React binding hosts JSON Forms with a precompiled-validator facade. Root validation delegates
