...
/// <summary>Builds management-tool actors exclusively from authenticated transport claims.</summary>
public sealed class ClaimsFormToolActorProvider : IFormToolActorProvider
{
    /// <summary>Holds the configured identity claim mappings.</summary>
    private readonly FormToolIdentityOptions options;

    /// <summary>Initializes the claims-based provider.</summary>
    public ClaimsFormToolActorProvider(FormToolIdentityOptions? options = null) => this.options = options ?? new FormToolIdentityOptions();
