...
Streamable HTTP transport from `ProgramKit.Mcp.AspNetCore`. It derives ownership from the transport
...
the selected Program Kit authentication profile owns scheme registration, token validation, and
authentication/authorization middleware. A canonical `permission:*` policy uses the mapping from
`ProgramKit.Authentication`. Bearer is the normal remote-client profile; BFF cookies require the
