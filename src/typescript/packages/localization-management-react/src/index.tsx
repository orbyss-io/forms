...
export function ProgramKitLocalizationManagement({ session, actor, labels: overrides, capabilities = {}, forms = [], pageSize = 50, maximumImportBytes = 8 * 1024 * 1024, importPreview, onChange, onAddRequested, onMessageAdded, onImportRequested, onPreviewImport, onExportRequested, onApplyImport, onLifecycleAction }: ProgramKitLocalizationManagementProps): ReactNode {
  ...
  return <section className="pk-localization">
    <div aria-label="Localization actions" className="pk-localization__toolbar" role="toolbar">
    ...
    <div aria-label="Translation filters" className="pk-localization__filters" role="search">
...
function LocalizationAddMessageDialog({ labels, forms, onCancel, onCreate }: { readonly labels: LocalizationManagementLabels; readonly forms: readonly LocalizationFormOption[]; readonly onCancel: () => void; readonly onCreate: (message: LocalizationMessage) => void }): ReactNode {
  ...
  return <dialog aria-labelledby={titleId} className="pk-localization__dialog" onCancel={event => { event.preventDefault(); onCancel(); }} ref={dialog}>
    <form method="dialog" onSubmit={submit}>
      <h2 id={titleId}>{labels.addTitle}</h2>
      {error.length > 0 && <p className="pk-localization__dialog-error" role="alert">{error}</p>}
      <div className="pk-localization__dialog-fields">
      ...
      <div className="pk-localization__dialog-actions"><button onClick={onCancel} type="button">{labels.cancel}</button><button type="submit">{labels.create}</button></div>
    </form>
  </dialog>;
}
...
function LocalizationImportDialog({ labels, forms, maximumBytes, onCancel, onPreview }: { readonly labels: LocalizationManagementLabels; readonly forms: readonly LocalizationFormOption[]; readonly maximumBytes: number; readonly onCancel: () => void; readonly onPreview: (submission: LocalizationImportSubmission) => Promise<void> }): ReactNode {
  ...
  return <dialog aria-labelledby={titleId} className="pk-localization__dialog" onCancel={event => { event.preventDefault(); if (!busy) onCancel(); }} ref={dialog}>
    <form onSubmit={event => { void submit(event); }}>
      ...
      <div className="pk-localization__dialog-actions"><button disabled={busy} onClick={onCancel} type="button">{labels.cancel}</button><button disabled={busy} type="submit">{labels.previewImport}</button></div>
    </form>
  </dialog>;
}
...
function LocalizationImportReview({ preview, labels, onApply }: { readonly preview: LocalizationImportPreview; readonly labels: LocalizationManagementLabels; readonly onApply: ((previewId: string) => void | Promise<void>) | undefined }): ReactNode {
  ...
  return <section aria-label={labels.importPreview} className="pk-localization__import"><h2>{labels.importPreview}</h2><p>{preview.mergePolicy} · {preview.changes.length} changes</p><ul>{preview.changes.map((change, index) => <li key={`${change.key}-${change.languageTag}-${index}`}>{change.kind}: {change.key} ({change.languageTag})</li>)}</ul>{onApply !== undefined && <button disabled={blocking} onClick={() => { void onApply(preview.previewId); }} type="button">{labels.applyImport}</button>}</section>;
}
