...
<#--
  Keycloak owns this provider screen. Keeping the entry template in the child theme makes its
  relative template import resolve through program-kit/keycloak.v2, so the consumer-owned stylesheet
  is loaded for passkey enrollment just as it is for the other login and required-action screens.
-->
<@layout.registrationLayout; section>
    ...
    <#elseif section = "header">
        ...
        ${msg("webauthn-registration-title")}
    <#elseif section = "form">
        <#-- The upstream base WebAuthn template resolves outside keycloak.v2's normal resource path. -->
        <link rel="stylesheet" href="${url.resourcesPath}/css/program-kit.css"/>
