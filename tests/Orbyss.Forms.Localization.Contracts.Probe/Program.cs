...
var localizationServices = new ServiceCollection();
new OrbyssFormsLocalizationFeature().ConfigureServices(localizationServices);
using (var localizationProvider = localizationServices.BuildServiceProvider(new ServiceProviderOptions { ValidateOnBuild = true, ValidateScopes = true }))
{
    Require(localizationProvider.GetRequiredService<IFormLocalizationBridge>() is DefaultFormLocalizationBridge, "localization feature did not register the Forms bridge");
}
