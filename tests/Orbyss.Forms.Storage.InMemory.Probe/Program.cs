...
new OrbyssFormsInMemoryStorageFeature().ConfigureServices(featureServices);
using (var featureProvider = featureServices.BuildServiceProvider(new ServiceProviderOptions { ValidateOnBuild = true, ValidateScopes = true }))
{
    Require(featureProvider.GetRequiredService<IFormDefinitionStore>() is InMemoryFormDefinitionStore, "in-memory feature did not register definition storage");
    var releaseStore = featureProvider.GetRequiredService<IFormReleaseStore>();
    Require(releaseStore is InMemoryFormReleaseStore && ReferenceEquals(releaseStore, featureProvider.GetRequiredService<IFormReleaseRetirementStore>()), "in-memory feature did not share release storage");
    Require(featureProvider.GetRequiredService<IFormSubmissionStore>() is InMemoryFormSubmissionStore, "in-memory feature did not register submission storage");
    Require(featureProvider.GetRequiredService<IFormAttachmentStore>() is InMemoryFormAttachmentStore, "in-memory feature did not register attachment storage");
    Require(featureProvider.GetRequiredService<IFormAttachmentContentStore>() is InMemoryFormAttachmentContentStore, "in-memory feature did not register attachment content storage");
}
