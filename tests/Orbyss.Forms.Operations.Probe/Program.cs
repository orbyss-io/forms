...
using Orbyss.Forms;
using Orbyss.Forms.Submissions.Mcp.AspNetCore;
...
try
{
    ...
    new OrbyssFormsSubmissionsFeature().ConfigureServices(composedBuilder.Services);
    await using (var composedApp = composedBuilder.Build())
    {
        var composedProvider = composedApp.Services;
        var submissionService = composedProvider.GetRequiredService<DefaultFormSubmissionService>();
        Require(ReferenceEquals(submissionService, composedProvider.GetRequiredService<IFormDraftOperations>()), "submissions feature did not share draft operations");
        Require(ReferenceEquals(submissionService, composedProvider.GetRequiredService<IFormDraftMigrationOperations>()), "submissions feature did not share draft migration operations");
        Require(ReferenceEquals(submissionService, composedProvider.GetRequiredService<IFormSubmissionOperations>()), "submissions feature did not share submission operations");
        Require(ReferenceEquals(submissionService, composedProvider.GetRequiredService<IFormSubmissionReview>()), "submissions feature did not share submission review");
        Require(composedProvider.GetRequiredService<IFormAttachmentOperations>() is DefaultFormAttachmentService, "submissions feature did not register attachment operations");
        Require(composedProvider.GetRequiredService<IFormAttachmentPolicy>() is RejectingFormAttachmentPolicy, "submissions feature must fail closed without an attachment allowlist");
        Require(composedProvider.GetRequiredService<IFormAttachmentScanner>() is RejectingFormAttachmentScanner, "submissions feature must fail closed without an attachment scanner");
    }
    ...
    var toolDraft = await FormOperationsTools.GetDraftAsync("draft-1", principal, new ClaimsFormSubmissionToolActorProvider(), restartAgain, CancellationToken.None);
    ...
    var shellSettings = new ShellSettings(new ShellId("forms-operations"), ["Orbyss.Forms.Web.Submissions", "Orbyss.Foundation.Mcp.AspNetCore", "Orbyss.Forms.Submissions.Mcp.AspNetCore"]);
    var webFeature = new OrbyssFormSubmissionsFeature(shellSettings);
    var mcpTransport = new FoundationMcpFeature(shellSettings);
    var mcpFeature = new OrbyssFormSubmissionsMcpFeature(shellSettings);
