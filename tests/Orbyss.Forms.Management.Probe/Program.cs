...
try
{
    ...
    var composedBuilder = WebApplication.CreateBuilder();
    new OrbyssFormsInMemoryStorageFeature().ConfigureServices(composedBuilder.Services);
    new OrbyssFormsJsonFormsFeature().ConfigureServices(composedBuilder.Services);
    new OrbyssFormsManagementFeature().ConfigureServices(composedBuilder.Services);
    await using (var composedApp = composedBuilder.Build())
    {
        var composedProvider = composedApp.Services;
        var composedCatalog = composedProvider.GetRequiredService<DefaultFormCatalogService>();
        Require(ReferenceEquals(composedCatalog, composedProvider.GetRequiredService<IFormAuthoring>()), "management feature did not share its catalog service with authoring");
        Require(ReferenceEquals(composedCatalog, composedProvider.GetRequiredService<IFormCatalogQueries>()), "management feature did not share its catalog service with queries");
        Require(ReferenceEquals(composedCatalog, composedProvider.GetRequiredService<IFormReleaseLifecycle>()), "management feature did not share its catalog service with release lifecycle");
        Require(composedProvider.GetRequiredService<IFormDefinitionValidator>() is FormDefinitionValidator, "JSON Forms feature did not register the default validator");
        Require(composedProvider.GetRequiredService<IFormCompiler>() is JsonFormsCompiler, "JSON Forms feature did not register the compiler");
        Require(composedProvider.GetRequiredService<IFormCompatibilityAnalyzer>() is DefaultFormCompatibilityAnalyzer, "management feature did not register compatibility analysis");
    }

    var storageSettings = new ShellSettings(new ShellId("filesystem-storage"), ["Orbyss.Forms.Storage.FileSystem"]);
    var fileSystemBuilder = WebApplication.CreateBuilder();
    new OrbyssFormsFileSystemStorageFeature(storageSettings).ConfigureServices(fileSystemBuilder.Services);
    await using (var fileSystemApp = fileSystemBuilder.Build())
    {
        var fileSystemProvider = fileSystemApp.Services;
        Require(fileSystemProvider.GetRequiredService<IFormDefinitionStore>() is FileSystemFormDefinitionStore, "filesystem feature did not register definition storage");
        Require(fileSystemProvider.GetRequiredService<IFormReleaseStore>() is FileSystemFormReleaseStore, "filesystem feature did not register release storage");
        Require(fileSystemProvider.GetRequiredService<IFormSubmissionStore>() is FileSystemFormSubmissionStore, "filesystem feature did not register submission storage");
        Require(fileSystemProvider.GetRequiredService<IFormAttachmentStore>() is FileSystemFormAttachmentStore, "filesystem feature did not register attachment storage");
        Require(fileSystemProvider.GetRequiredService<IFormAttachmentContentStore>() is FileSystemFormAttachmentContentStore, "filesystem feature did not register attachment content storage");
    }
