...
using ProgramKit.Forms.Mcp.AspNetCore;
using ProgramKit.Forms.Tool;
using ProgramKit.Forms.Web.Submissions;
using ProgramKit.Mcp.AspNetCore;

var root = Path.Combine(Path.GetTempPath(), "program-kit-form-operations-" + Guid.NewGuid().ToString("N"));
...
try
{
    ...
    var shellSettings = new ShellSettings(new ShellId("forms-operations"), ["ProgramKit.Forms.Web.Submissions", "ProgramKit.Mcp.AspNetCore", "ProgramKit.Forms.Mcp.AspNetCore"]);
    var webFeature = new ProgramKitFormSubmissionsFeature(shellSettings);
    var mcpTransport = new ProgramKitMcpFeature(shellSettings);
    var mcpFeature = new ProgramKitFormMcpFeature(shellSettings);
    ...
    await using (var app = builder.Build())
    {
        ...
        try
        {
            ...
            using var anonymousApi = await webClient.GetAsync("/program-kit/forms/drafts");
            Require(anonymousApi.StatusCode == System.Net.HttpStatusCode.Unauthorized, "form API allowed anonymous access");
            using var anonymousMcp = await webClient.PostAsync("/program-kit/mcp", new StringContent("{}", Encoding.UTF8, "application/json"));
    ...
    Console.WriteLine("Program Kit form operations probe passed.");
}
finally
{
