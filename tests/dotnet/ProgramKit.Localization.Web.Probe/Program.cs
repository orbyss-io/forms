...
using ProgramKit.Localization.Web.Management;
using ProgramKit.Localization.Web.Runtime;
...
var settings = new ShellSettings(new ShellId("localization"), ["ProgramKit.Localization.Web.Management", "ProgramKit.Localization.Web.Runtime"]);
settings.ConfigurationData[$"{LocalizationManagementWebOptions.SectionName}:MaximumImportBytes"] = "1024";
var managementFeature = new ProgramKitLocalizationManagementFeature(settings);
var runtimeFeature = new ProgramKitLocalizationRuntimeFeature(settings);
...
try
{
    ...
    using var unauthorized = await client.GetAsync("/_program-kit/localization/catalogs");
    Require(unauthorized.StatusCode == HttpStatusCode.Unauthorized, "management endpoint allowed an anonymous request");

    client.DefaultRequestHeaders.Add("X-Test-Auth", "true");
    using var catalogs = await client.GetAsync("/_program-kit/localization/catalogs");
    Require(catalogs.StatusCode == HttpStatusCode.OK, "authenticated catalog query failed");
    using var catalogResponse = await client.GetAsync("/_program-kit/localization/catalogs/application");
    ...
    using var preview = await client.PostAsJsonAsync("/_program-kit/localization/catalogs/application/imports/preview", upload);
    ...
    using var oversized = await client.PostAsJsonAsync("/_program-kit/localization/catalogs/application/imports/preview", oversizedUpload);
    ...
    using var rejectedBody = await client.PostAsJsonAsync("/_program-kit/localization/catalogs/application/imports/preview", oversizedBody);
    ...
    using var apply = await client.PostAsJsonAsync("/_program-kit/localization/catalogs/application/imports/preview/apply", new LocalizationImportApplyModel(mutation));
    ...
    using var exported = await client.PostAsJsonAsync("/_program-kit/localization/catalogs/application/exports", new LocalizationExportWebRequest(LocalizationImportFormat.Json, scope, ["nl"], SuggestedBaseName: "application"));
    ...
    using var bundle = await client.GetAsync("/_program-kit/localization/runtime/bundles/Application/nl");
    ...
    using var cachedRequest = new HttpRequestMessage(HttpMethod.Get, "/_program-kit/localization/runtime/bundles/Application/nl");
    ...
    using var resolved = await client.GetAsync("/_program-kit/localization/runtime/messages/Application/nl/welcome");
