...
def main() -> int:
    ...
    forms = root / "src/Orbyss.Forms.Storage.InMemory"
    source = "\n".join(path.read_text(encoding="utf-8") for path in forms.rglob("*") if path.suffix in {".cs", ".csproj"})
    for forbidden in ("EntityFrameworkCore", "Microsoft.Data.Sqlite", "Azure.", "Amazon.", "IWebShellFeature", "InternalsVisibleTo"):
        if forbidden in source:
            raise AssertionError(f"{forms.name} owns a provider, web boundary, or privileged test seam: {forbidden}")
    ...
    print("Orbyss Forms in-memory persistence validation passed.")
    return 0
